...
func main() {
	...
	sol := solariClient()
	notaryCore := service.NewNotaryCore(st.Events(), sealer)
	...
	ledgerv1.RegisterCaptureServiceServer(srv, service.NewCapture(st.Sessions(), st.Grants(), verifier, notaryCore, idgen, clk))
