...
import { StatusPill, Copyable, MonospaceHash, Json, EmptyState } from "@/lib/ui";
import { IconSearch, IconCheck, IconFileText } from "@/lib/icons";
...
export function Activity() {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div
        className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 rounded-babit shadow-xs"
        style={{
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto">
          {(["ALL", "VERIFIED", "PENDING", "FAILED"] as const).map((s) => (
            <button
              ...
              style={{
                backgroundColor: statusFilter === s ? "var(--fg)" : "var(--secondary)",
                color: statusFilter === s ? "var(--surface)" : "var(--muted)",
                border: `1px solid ${statusFilter === s ? "var(--fg)" : "var(--border)"}`,
              }}
            >
              {s}
            </button>
      ...
      {/* Activity Data Table or Empty State */}
      {filtered.length === 0 ? (
        <EmptyState
          title="No activity events found"
          description="Try adjusting your filter status or search criteria."
              ...
              <tbody className="divide-y text-xs" style={{ borderColor: "var(--border-subtle)" }}>
                {filtered.map((row) => (
                  <tr
                    ...
                    style={{
                      color: "var(--fg)",
                      backgroundColor: selectedItem?.id === row.id ? "var(--secondary)" : "transparent",
                    }}
                  >
                    <td className="px-5 py-3.5 font-semibold font-sans">{row.actionName}</td>
      ...
      {selectedItem && (
        <div
          className="fixed inset-y-0 right-0 w-full sm:w-[500px] shadow-2xl z-50 p-6 overflow-y-auto space-y-6 animate-fade-in"
          style={{
            backgroundColor: "var(--surface)",
        ...
        >
          ...
          <div className="flex items-center justify-between pb-4" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-semibold font-mono" style={{ color: "var(--fg)" }}>{selectedItem.actionName}</h2>
                <StatusPill status={selectedItem.status} />
              </div>
              <span className="text-[11px] font-mono block mt-0.5" style={{ color: "var(--muted)" }}>{selectedItem.timestamp}</span>
            </div>
            <button
              onClick={() => setSelectedItem(null)}
              className="p-1 rounded-babit-sm cursor-pointer"
          ...
          {/* Tabs */}
          <div className="flex items-center gap-1 pb-2 font-mono text-xs" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
            {(["overview", "authority", "evidence", "technical"] as const).map((t) => (
              <button
                ...
                style={{
                  backgroundColor: detailTab === t ? "var(--fg)" : "transparent",
                  color: detailTab === t ? "var(--surface)" : "var(--muted)",
          ...
          <div className="space-y-4 font-mono text-xs">
            {detailTab === "overview" && (
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Action ID</span>
                  <span className="text-sm font-semibold" style={{ color: "var(--fg)" }}>{selectedItem.id}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Subject Agent</span>
                  <span style={{ color: "var(--fg)" }}>{selectedItem.agent}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Target Resource</span>
                  <span className="break-all" style={{ color: "var(--fg)" }}>{selectedItem.resource}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Execution Surface</span>
                  <span style={{ color: "var(--fg)" }}>{selectedItem.surface}</span>
                </div>
              </div>
            )}
            ...
            {detailTab === "authority" && (
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Human Supervisor Principal</span>
                  <span className="font-semibold" style={{ color: "var(--fg)" }}>{selectedItem.principal}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Grant Ticket Reference</span>
            ...
            {detailTab === "evidence" && (
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Event SHA-256 Hash</span>
                  <MonospaceHash hash={selectedItem.eventHash} />
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Sequential Prev Hash</span>
                  <MonospaceHash hash={selectedItem.prevHash} />
                </div>
                <div>
                  <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Notary Ed25519 Signature</span>
                  <span className="text-[11px] break-all block" style={{ color: "var(--muted)" }}>{selectedItem.notarySig}</span>
                </div>
              </div>
            )}
            ...
            {detailTab === "technical" && (
              <div className="space-y-2">
                <span className="text-[10px] uppercase block" style={{ color: "var(--muted)" }}>Raw Action Payload JSON</span>
