    ...
    [
      {
        ...
        desc: "Validate receipts and proofs.",
        icon: (
      ...
      },
      {
        tab: "projects",
        title: "Projects",
        desc: "Track work tied to ledger actions.",
        icon: <IconFolder className="w-4 h-4" />,
      },
      {
        tab: "settings",
        title: "Settings",
        desc: "Edit account and workspace details.",
    ...
    <div className="space-y-6">
      ...
      {isAuthenticated && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            label="Actions notarized"
            value={n(stats?.total_events).toLocaleString()}
            icon={<IconActivity className="w-4 h-4" />}
          />
          <MetricCard
            label="Sessions"
            value={n(stats?.total_sessions).toLocaleString()}
            icon={<IconLayers className="w-4 h-4" />}
          />
          <MetricCard
            label="Active grants"
            value={Math.max(0, n(stats?.total_grants) - n(stats?.revoked_grants)).toLocaleString()}
            icon={<IconGitBranch className="w-4 h-4" />}
          />
          <button
          ...
          <div className="grid gap-4 sm:grid-cols-[auto_1fr]">
            {keyId && (
              <div className="space-y-1.5">
                <span
                  className="text-[10px] uppercase tracking-wide font-mono block"
                  style={{ color: "var(--muted)" }}
                >
                  Key ID
                </span>
                <span className="font-mono tnum text-xs" style={{ color: "var(--fg)" }}>
                  {keyId}
                </span>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((a) => (
            ...
            >
              ...
              <p className="mt-3 text-sm font-semibold" style={{ color: "var(--fg)" }}>
                {a.title}
              </p>
